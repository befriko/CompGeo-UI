...
% first need to regrid picked values to common sample intervals
numpicks = 20; % number of picks per velocity location for the 1-D interp
tlin = linspace(tmin,tmax,m)'; tlin = tlin*t_ones;
tlin_temp = linspace(tmin,tmax,numpicks)'; tlin_temp = tlin_temp*t_ones;
for i = 1:n
    ...
    % vertically resample the data to have better population before 2-D
    % interpolation. This helps in producing better vertical resolution
    vrms_temp(:,i) = interp1(tnmo(:,i),vnmo(:,i),tlin_temp(:,i),'pchip','extrap');
    vint_temp(:,i) = interp1(tnmo(:,i),vint(:,i),tlin_temp(:,i),'pchip','extrap');
end
...
for i = 1:n
    vrms_interp(:,i) = interp1(tlin_temp(:,i),vrms_temp(:,i),t_interp(:,i),'pchip','extrap');
    vint_interp(:,i) = interp1(tlin_temp(:,i),vint_temp(:,i),t_interp(:,i),'pchip','extrap');
...
vrms_interp_2d_1pass = interp2(x(:,1),tlin_temp(:,1),vrms_temp,x_interp(:,1)',t_interp(:,1),'cubic');
vint_interp_2d_1pass = interp2(x(:,1),tlin_temp(:,1),vint_temp,x_interp(:,1)',t_interp(:,1),'cubic');

clear vnmo_temp vint_temp tlin_temp % clean up

% display 1-D and 2-D interpolated rms velocity
figure;subplot(3,1,1);imagesc(x(:,1),tlin(:,1),vrms_grid,[min(min(vrms_grid)) max(max(vrms_grid))]);colorbar;
xlabel('Distance (m)');ylabel('Time (s)');colormap(jet);
title('Regridded picked rms velocity');
subplot(3,1,2);imagesc(x_interp(:,1),t_interp(:,1),vrms_interp_2d,[min(min(vrms_grid)) max(max(vrms_grid))]);colorbar;
xlabel('Distance (m)');ylabel('Time (s)');colormap(jet);
title('Two-pass 1-D interpolated rms velocity');
subplot(3,1,3);imagesc(x_interp(:,1),t_interp(:,1),vrms_interp_2d_1pass,[min(min(vrms_grid)) max(max(vrms_grid))]);colorbar;
xlabel('Distance (m)');ylabel('Time (s)');colormap(jet);
title('One-pass 2-D interpolated rms velocity');

% display 1-D and 2-D interpolated interval velocity
figure;subplot(3,1,1);imagesc(x(:,1),tlin(:,1),vint_grid,[min(min(vint_grid)) max(max(vint_grid))]);colorbar;
xlabel('Distance (m)');ylabel('Time (s)');colormap(jet);
title('Regridded picked interval velocity');
subplot(3,1,2);imagesc(x(:,1),t_interp(:,1),vint_interp_2d,[min(min(vint_grid)) max(max(vint_grid))]);colorbar;
xlabel('Distance (m)');ylabel('Time (s)');colormap(jet);
title('Two-pass 1-D interpolated interval velocity');
subplot(3,1,3);imagesc(x_interp(:,1),t_interp(:,1),vint_interp_2d_1pass,[min(min(vint_grid)) max(max(vint_grid))]);colorbar;
xlabel('Distance (m)');ylabel('Time (s)');colormap(jet);
