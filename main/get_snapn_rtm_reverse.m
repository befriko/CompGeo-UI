function [cresult_sum,cresult_sum_source,cresult_sum_receiver,cresult_sum_source_normalization,cresult_sum_receiver_normalization]=get_snapn_rtm_reverse(delx,delt,velocity,snap1,snap2,toutput,laplacian,boundary,shot,zs)
...
% edited by Wenyong Pan, March 2013
%
...
ntimg=floor(round(toutput/delt));
kk=1;
...
for k=1:round(toutput/delt)
   ...
   snap2(zs,:) = shot(round(toutput/delt)-k+1,:);
   [snapshotn]=get_snap_reverse(delx,delt,velocity,snap1,snap2,laplacian,boundary);
   snap1=snap2;
   snap2=snapshotn;
...
% imaging
      ...
      kk=kk+1;
      uread=fread(fid,[nx,nz],'float');
      
      cresult=snapshotn.*uread;
      source=uread.*uread;
      
      cresult_sum=cresult_sum+cresult;
      cresult_sum_source=cresult_sum_source+source;
      cresult_sum_receiver=cresult_sum_receiver+snapshotn.*snapshotn;
      
      figure(3);cla % display imaging process
      set(gcf,'color','w');
      subplot(1,3,1)
      imagesc(xxx,zzz,uread);
      xlabel('Distance (m)','fontsize',12,'fontweight','bold');
      ylabel('Depth (m)','fontsize',12,'fontweight','bold');
      title(strcat('Fwd. Modeling Wavefields at ',num2str(toutput-k*delt),' s'),'fontsize',12,'fontweight','bold');
      colormap('jet');     
      subplot(1,3,2)
      imagesc(xxx,zzz,snapshotn);
      xlabel('Distance (m)','fontsize',12,'fontweight','bold');
      ylabel('Depth (m)','fontsize',12,'fontweight','bold');
      title('Back-propagated wavefield','fontsize',12,'fontweight','bold');
      colormap('jet');           
      subplot(1,3,3)
      imagesc(xxx,zzz,cresult_sum);
      xlabel('Distance (m)','fontsize',12,'fontweight','bold');
      ylabel('Depth (m)','fontsize',12,'fontweight','bold');
      title('Cross-correlation','fontsize',12,'fontweight','bold');
      colormap('jet');     
      pause(0.001)
    
%    if (round(toutput/delt)-k)==500
%     reverse=snapshotn;
%     save reverse.mat reverse
%    end
   
end
cresult_sum_source_normalization=cresult_sum_source_normalization+cresult_sum./(cresult_sum_source+0.5*max(max(cresult_sum_source)));
cresult_sum_receiver_normalization=cresult_sum_receiver_normalization+cresult_sum./(cresult_sum_source+cresult_sum_receiver);

% save imaging_result.mat cresult_sum
% save imaging_result2.mat cresult_sum_source
% save imaging_result3.mat cresult_sum_source_normalization
toc
